...
const ComLook = ({ route }) => {
  ...
  const Content = (prop) => {
    ...
    return (
      <View style={{ marginTop: nowmargin, marginLeft: 10 }}>
        <Image source={{ uri: 'https://pluslink.kr/data/member_image/' + prop.img.substring(0, 2) + '/' + prop.img + '.gif' }} style={{ width: chartWidth - 20, height: 200, borderRadius: 15, backgroundColor: 'gray' }}></Image>
        ...
        <Text style={{ margin: 10, width: chartWidth - 40 }}>{prop.content.replace(/\r\n/g, '')}</Text>
        ...
        <View style={{ width: chartWidth, borderWidth: 0.5, borderColor: 'gray', marginLeft: -10 }}></View>
      </View>
    )
  }
        ...
        <ScrollView style={{ backgroundColor: 'white' }}>
          <View style={{ marginBottom: 100 }}>
            ...
            {selectmenu == '시공' ?
              ...
              // </ScrollView> 
              :
              <View style={{ justifyContent: 'center', alignItems: 'center', marginTop: 10, marginBottom: 80 }}>
  ...
  return (
    <View style={{ marginTop: 20 }}>
      <TouchableOpacity onPress={() => navigation.navigate('베스트보기', { id: prop.id })}>
        <View style={{ width: chartWidth - 20, marginLeft: 10 }}>
          <Image source={{ uri: prop.img }} style={{ width: chartWidth - 20, height: 200, backgroundColor: 'gray', borderRadius: 10 }}></Image>
          <Text numberOfLines={1} style={{ fontWeight: 'bold', fontSize: 18, margin: 15 }}>{prop.title}</Text>
          <Text numberOfLines={2} style={{ marginLeft: 15, marginRight: 15, marginBottom: 15, fontWeight: '100' }}>{prop.content}</Text>
          <View style={{ width: chartWidth - 20, borderWidth: 0.5, borderColor: 'gray', marginTop: 5 }}></View>
        </View>
      </TouchableOpacity>
    </View>
  )
...
const ReviewItem = (prop) => {
  return (
    <View style={{ width: chartWidth, borderWidth: 0.4, borderColor: 'gray', borderRadius: 10, marginTop: 10 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', width: chartWidth - 20, margin: 10 }}>
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          <Image style={{ width: 30, height: 30, borderRadius: 28, marginLeft: 5 }} source={{ uri: 'https://pluslink.kr/data/member_image/' + prop.id.substr(0, 2) + '/' + prop.id + '.gif' }}></Image>
